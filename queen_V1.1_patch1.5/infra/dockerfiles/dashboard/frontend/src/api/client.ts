...
import type {
  ...
  TimelineResponse, Settings, RoleDef, SkillDef,
} from "./types";
...
export const roles = {
  list: () => get<RoleDef[]>("/roles"),
  get: (name: string) => get<RoleDef>(`/roles/${name}`),
  reload: () => post<{ loaded: number }>("/roles/reload"),
};

export const skills = {
  list: () => get<SkillDef[]>("/skills"),
  get: (name: string) => get<{ name: string; content: string }>(`/skills/${name}`),
  reload: () => post<{ loaded: number }>("/skills/reload"),
};
