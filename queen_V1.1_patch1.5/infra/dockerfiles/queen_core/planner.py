...
Décompose cet objectif en jobs exécutables."""
    ...
    system_base = PLANNING_SYSTEM.format(max_jobs=budget['max_jobs'])

    # Patch 1.2: rôle optionnel (roles/) pour specialiser le planner
    role_name = constraints.get("role", "")
    if role_name:
        try:
            role = get_role_registry().get(role_name)
            system = role.build_system_prompt(extra_context=system_base) if role else system_base
        except Exception:
            system = system_base
    else:
        system = system_base
