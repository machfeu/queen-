...
def generate(prompt: str, system: str = "", temperature: float = 0.7,
    ...
    If run_id is provided, records token usage in budget tracker.
    Raises RuntimeError on persistent failure.
    """
    for attempt in range(LLM_MAX_RETRIES + 1):
        try:
            if LLM_PROVIDER == "ollama":
                text, usage = _ollama_generate(prompt, system, temperature, max_tokens, json_mode)
            else:
                text, usage = _openai_generate(prompt, system, temperature, max_tokens, json_mode)

            # Track budget if run_id provided
            if run_id and usage:
                try:
...
def generate_json(prompt: str, system: str = "", temperature: float = 0.3,
                  max_tokens: int = 4096, run_id: str = "") -> Dict[str, Any]:
    """Generate and parse JSON from LLM."""
    raw = generate(prompt, system=system, temperature=temperature,
                   max_tokens=max_tokens, json_mode=True, run_id=run_id)
...
def _openai_generate(prompt: str, system: str, temperature: float,
    ...
    usage = {
        "input_tokens": api_usage.get("prompt_tokens", len(prompt) // 4),
        "output_tokens": api_usage.get("completion_tokens", len(text) // 4),
    }
    return text, usage
