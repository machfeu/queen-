...
def validate_goal_constraints(constraints: Dict[str, Any]) -> Dict[str, Any]:
    ...
    cleaned = {}
    cleaned["timeout"] = min(
        constraints.get("timeout", DEFAULT_JOB_TIMEOUT),
        MAX_JOB_TIMEOUT
    )
    cleaned["max_output_bytes"] = min(
        constraints.get("max_output_bytes", MAX_OUTPUT_BYTES),
        MAX_OUTPUT_BYTES
    )
    cleaned["risk_level"] = constraints.get("risk_level", "medium")
    if cleaned["risk_level"] not in RISK_LEVELS:
        cleaned["risk_level"] = "medium"
    cleaned["repo_target"] = constraints.get("repo_target", "default")
    cleaned["success_criteria"] = constraints.get("success_criteria", "")
    cleaned["require_manual_approval"] = constraints.get("require_manual_approval", True)
    return cleaned
